...
## 缓存有关的Headers
### 强缓存
#### [Cache-Control](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers/Cache-Control) (http1.1，响应头)
**与Expires同时出现时，以`Cache-Control`为准**
...
no-cache：每次在使用缓存之前都强制发送请求给源服务器进行验证，检查文件该没改变(其实这里和ETag/Last区别不大)
